...
egg_groups = pd.read_csv("egg groups.csv")
...
g.add( (prop["has_resistence.html"], RDF.type, RDF.Property) )
g.add( (prop["has_resistence.html"], RDFS.domain, rdf_pok) )
g.add( (prop["has_resistence.html"], RDFS.range, rdf_res) )
...
for row in poke.itertuples():
    ...
    for i in range(2,20):
        resistence = URIRef("http://pokeres/"+against[i-2]+"/"+str(row[i]))
        g.add( (pok[nome], prop["has_resistence.html"], resistence) )
        g.add( (resistence, RDF.type, rdf_res) )
        g.add( (resistence, prop["res_value.html"], Literal(row[i], datatype=XSD.float)) )
        el = URIRef(re.split('\_',against[i-2])[1].capitalize() + "_(type)")
        g.add( (resistence, prop["res_type.html"], typ[el]) )
