...
#query per estrarre tutti i pokemon
...
#richiedo un pokemon casualmente
...
#query per estrarre il tipo del pokemon casuale
...
#query per estrarre tutti i tipi esclusi quelli del pokemon random
...
#estraggo 3 tipi diversi da quello del pokemon casuale
...
#query per estrarre la velocità
...
#estraggo le velocità, e assegno i punteggi
...
#query per estrarre ability del pokemon casuale
...
#query per estrarre tutti le ability esclusi quelli del pokemon random
...
#estraggo 3 ab diversi da quello del pokemon casuale
...
#estraggo pokemon diversi da quello casuale e dalla sua evolution
...
#crea il json delle domande per l'html
...
def get_type_coeff(x):
    if x["Resistence_Total"]==0: return 5
    return 1 - math.log(float(x["Resistence_Total"]))/math.log(2)/5
...
def get_pokemon_list(pokemon_to_fight, tipo1, tipo2):
    ...
    tabella = pd.DataFrame.from_records(pokemon, columns=["Pokemon","Resistence_Total"])
    if tipo2 != "Nessuno":
        q_supereff = g.query(
            """SELECT DISTINCT ?pokemon ?res_val
               WHERE {
        ...
        for row in q_supereff.result:
            ...
            #print(re.split('\(\'|\'\)|\',|\(\"|\"\"\)', row))
            pokemon.append( re.split('\(\'|\'\)|\',|\(\"|\"\)', row)[1:4:2])
        tipo_2 = pd.DataFrame.from_records(pokemon, columns=["Pokemon","Resistence_2"])
        tabella = pd.merge(left= tabella, right = tipo_2, on="Pokemon")
        tabella["Resistence_Total"] = tabella.apply(lambda x:float(x["Resistence_Total"]) * float(x["Resistence_2"]), axis=1)
    ...
    #tabella["coeff"] = tabella["coeff"].apply(lambda x: x**2)
    ...
    while (pokemon_lista == None or len(pokemon_lista) < 10):
        ...
        if (pokemon_lista != None and pokemon_to_add in pokemon_lista):
            ...
            #print("Pokemon trovato")
            #print(len(pokemon_lista), pokemon_to_add)
